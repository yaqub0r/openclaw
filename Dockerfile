...
RUN --mount=type=cache,id=openclaw-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    ...
    DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
      procps hostname curl git lsof openssl cron
...
COPY --from=runtime-assets --chown=node:node /app/scripts/gateway-entrypoint.sh ./scripts/gateway-entrypoint.sh
...
# Entrypoint starts cron (as root) and then drops privileges to `node`.
USER root
RUN chmod 755 /app/scripts/gateway-entrypoint.sh
